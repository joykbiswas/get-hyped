...
export default function Navbar() {
  ...
  return (
    <>
      <nav 
        className="fixed top-0 left-0 w-full z-50 flex items-center justify-between px-6 md:px-8 py-4 bg-[#FAF4EC] shadow-sm"
        style={{
      ...
      >
        ...
        <Link 
          href="/" 
          className="flex items-center h-10 md:h-12 transition-opacity hover:opacity-85"
        ...
        <div className="hidden lg:flex items-center gap-1 mr-6">
          {navLinks.map((link) => (
            <NavSwooshLink key={link.name} href={link.href}>
              {link.name}
            </NavSwooshLink>
          ))}
        </div>
        ...
        <button
          ...
          className="lg:hidden flex flex-col justify-center items-center w-10 h-10 relative z-[1100]"
      ...
      {/* Mobile Menu Drawer */}
      <div 
        className={`fixed top-0 right-0 h-full w-[70%] max-w-[360px] bg-[#FAF4EC] z-45 transition-transform duration-400 ease-[cubic-bezier(0.2,0.9,0.4,1.1)] lg:hidden ${
          isMenuOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
        style={{ boxShadow: '-5px 0 25px rgba(0,0,0,0.05)' }}
      >
        <div className="flex flex-col h-full pt-24 pb-8 px-6">
          ...
          {/* Mobile Get Results Button */}
          <div className="mt-auto pt-8">
            <GetResultsButtonMobile />
          </div>
        </div>
      </div>
    </>
...
function NavSwooshLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
    ...
    >
      <span className="absolute inset-0 flex pointer-events-none">
        <span className="flex-1 h-full bg-black transition-transform duration-400 ease-[cubic-bezier(0.2,0.9,0.4,1.1)] scale-x-0 group-hover:scale-x-100 origin-left delay-75" />
        <span className="flex-1 h-full bg-black transition-transform duration-400 ease-[cubic-bezier(0.2,0.9,0.4,1.1)] scale-x-0 group-hover:scale-x-100 origin-left" />
      </span>
      <span className="relative z-10 px-5 py-2.5 text-base font-medium text-black group-hover:text-white transition-colors duration-250">
        {children}
      </span>
    </Link>
  );
...
function GetResultsButton() {
  return (
    <button className="relative inline-flex items-center justify-center overflow-hidden rounded-full bg-black group">
      <span className="absolute inset-0 bg-gray-700 transition-transform duration-300 scale-x-0 group-hover:scale-x-100 origin-left rounded-full" />
      <span className="relative z-10 flex items-center gap-2 px-6 py-2.5">
...
// Mobile Get Results Button
function GetResultsButtonMobile() {
  return (
    <button className="w-full relative inline-flex items-center justify-center overflow-hidden rounded-full bg-black group">
      <span className="absolute inset-0 bg-gray-700 transition-transform duration-300 scale-x-0 group-hover:scale-x-100 origin-left rounded-full" />
