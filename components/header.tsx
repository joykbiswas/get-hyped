    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled ? "bg-[#FAF4EC]/90 backdrop-blur-md shadow-sm" : "bg-[#FAF4EC]"
        }`}
      >
        <div className="px-4 md:px-8 py-4 flex items-center justify-between">
          {/* Logo - Your Original SVG */}
          ...
          <nav className="hidden md:flex items-center gap-2">
            {navItems.map((item) => (
              <NavSwooshLink key={item.label} href={item.href}>
                {item.label}
              </NavSwooshLink>
            ))}
            <a
              href="#contact"
              className="ml-2 px-5 py-2.5 rounded-full bg-[#1a1a1a] text-white font-medium text-sm flex items-center gap-2 hover:bg-[#333] transition-colors"
            >
              Get Results
              <ArrowIcon />
            </a>
          ...
          <button
            ...
            className="md:hidden w-10 h-10 rounded-2xl bg-[#fcb8fa] flex items-center justify-center"
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-[#fcb8fa]"
          >
            <div className="p-4 h-full flex flex-col">
              ...
              <nav className="flex-1 flex flex-col items-center justify-center gap-4">
                {navItems.map((item, index) => (
                  <motion.a
                    ...
                    className="px-8 py-4 rounded-full bg-white text-[#1a1a1a] font-semibold text-xl shadow-sm hover:shadow-md transition-shadow"
                  >
                    {item.label}
                  </motion.a>
                ))}
              ...
              >
                <a
                  href="#contact"
                  onClick={() => setIsMenuOpen(false)}
                  className="px-8 py-4 rounded-full bg-[#1a1a1a] text-white font-semibold text-xl flex items-center gap-3 hover:bg-[#333] transition-colors"
                >
                  Get Results
                  <ArrowIcon />
                </a>
...
// Logo Component - Your Original SVG
function LogoIcon() {
  return (
    <svg 
  ...
  return (
    ...
    >
      <span className="absolute inset-0 flex pointer-events-none rounded-full">
        <span className="flex-1 h-full bg-black transition-transform duration-400 ease-[cubic-bezier(0.2,0.9,0.4,1.1)] scale-x-0 group-hover:scale-x-100 origin-left delay-75 rounded-l-full" />
        <span className="flex-1 h-full bg-black transition-transform duration-400 ease-[cubic-bezier(0.2,0.9,0.4,1.1)] scale-x-0 group-hover:scale-x-100 origin-left rounded-r-full" />
      </span>
      <span className="relative z-10 px-5 py-2.5 text-sm font-medium text-black group-hover:text-white transition-colors duration-250">
        {children}
      </span>
    </a>
  )
